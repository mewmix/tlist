...
function store (state, emitter) {
  ...
  emitter.on('createDoc', docName => {
    ...
    archive.ready(() => {
      ...
      state.key = key
      state.archive = archive
      let shoppingList = ['https://bloks.telos.io', 'https://tlis.glitch.me', 'https://t.me/hellotelos', 'https://telos.studio.io']
      writeDatJson(() => {
      ...
      function writeDatJson (cb) {
        const json = JSON.stringify({
          url: `dat://${keyHex}/`,
          title: docName,
          description: `Dat TList - https://${state.glitchAppName}.glitch.me/`
        }, null, 2)
  ...
  function readShoppingList () {
    ...
    archive.readdir('/shopping-list', (err, fileList) => {
      if (err) {
        ...
        state.error = 'Error loading list'
        emitter.emit('render')
        return
      }
